...
export default class LoginPage extends React.Component{
  ...
  constructor(props) {
    ...
    if(UserService.currentUserValue) { //Check if the current user exists or not 
      this.props.history.push('/');
    }

    this.state = {  // create variables 
      user: new User('',''),
      submitted: false,
      loading: false,
      errorMessage: ''
  ...
  handleLogin(e) {
    ...
    UserService.login(user).then(data => {
      this.props.history.push("/home"); //after successful login go to home
    }, error => {
      this.setState({
        errorMessage: "Username or password is not valid", //if not successful error
        loading: false
      });
    });
  }

  render() {  //here goes the html form for login
